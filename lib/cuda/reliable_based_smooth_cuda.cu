...
__global__ void reliable_based_smooth_cuda_kernel(
    const scalar_t *__restrict__ reliable_area,
  ...
  if(index<N && (dense_mode || grad[index]!=0)) {
    ...
    grad[index] += reliable_area[index]*grad_to_add;
  }
}
...
void reliable_based_smooth_cuda(torch::Tensor reliable_area,torch::Tensor param, torch::Tensor grad, float wx, float wy, float wz, bool dense_mode) {
  ...
  if(dense_mode) {
    AT_DISPATCH_FLOATING_TYPES(param.type(), "reliable_based_smooth_cuda", ([&] {
      reliable_based_smooth_cuda_kernel<scalar_t,true><<<blocks, threads>>>(
          reliable_area.data_ptr<scalar_t>(),
          param.data<scalar_t>(),
          grad.data<scalar_t>(),
          wx, wy, wz,
          sz_i, sz_j, sz_k, N);
     ...
     AT_DISPATCH_FLOATING_TYPES(param.type(), "reliable_based_smooth_cuda", ([&] {
      reliable_based_smooth_cuda_kernel<scalar_t,false><<<blocks, threads>>>(
          reliable_area.data_ptr<scalar_t>(),
          param.data<scalar_t>(),
          grad.data<scalar_t>(),
          wx, wy, wz,
