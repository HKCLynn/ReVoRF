...
file_path = "index.txt"  # 文件路径
file_mode = "a"  # 打开文件的模式，使用 "w" 表示写入模式
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    # c2w_test =poses[i_test]
    # dists_test = np.sum(np.square(c2w_test[:3,3] - poses[:,:3,3]), -1)
    # i_index = np.argsort(dists_test)
    # with open(file_path, "a") as file:
    #     file.write(str(basedir)+'['+str(i_index[1])+','+str(i_index[2])+','+str(i_index[3])+']\n')
    # exit(0)
