...
def segment_entropy(src: torch.Tensor, index: torch.Tensor, alphainv_last:torch.Tensor, entropy_type:str,
                    threshold:float=1e-3, dim: int = -1, 
                    dim_size: Optional[int] = None) -> torch.Tensor:
...
def compute_relative_depth_loss(render_depths, gt_depths, patch_num, patch_size, mask=-1):
    predict_depths = render_depths.reshape(patch_num,patch_size,patch_size)
    relative_depth_loss = 0
    if mask==-1:
        pair_num=2*patch_size
    else:
        pair_num=8*patch_size
    patch_num_based = np.tile(np.arange(0, patch_num),pair_num)
    select_xy1=np.random.randint(0, patch_size, (pair_num*patch_num,2))
    select_bxy1=np.concatenate([patch_num_based[...,None],select_xy1],axis=-1)
    
    select_xy2=np.random.randint(0, patch_size, (pair_num*patch_num,2))
    select_bxy2=np.concatenate([patch_num_based[...,None],select_xy2],axis=-1)
    
    predict_sub1=predict_depths[select_bxy1[:,0],select_bxy1[:,1],select_bxy1[:,2]]
    predict_sub2=predict_depths[select_bxy2[:,0],select_bxy2[:,1],select_bxy2[:,2]]

    gt_sub1=gt_depths[select_bxy1[:,0],select_bxy1[:,1],select_bxy1[:,2]]
    gt_sub2=gt_depths[select_bxy2[:,0],select_bxy2[:,1],select_bxy2[:,2]]

    gt_mask=gt_sub1>gt_sub2
    if mask==-1:
        relative_depth_loss=(gt_mask*(torch.clamp(predict_sub1-predict_sub2+1e-3, min=0))).sum()/(gt_mask.sum().item()+1e-6)
    else:
        unreliable_masks1 = ~(mask[select_bxy1[:,0],select_bxy1[:,1],select_bxy1[:,2]])
        unreliable_masks2 = ~(mask[select_bxy2[:,0],select_bxy2[:,1],select_bxy2[:,2]])
        unreliable_masks = torch.logical_and(unreliable_masks1,unreliable_masks2)
        relative_depth_loss=(gt_mask*unreliable_masks*(torch.clamp(gt_sub1-gt_sub2+1e-3, min=0))).sum() \
        /((gt_mask*unreliable_masks).sum().item()+1e-6)
