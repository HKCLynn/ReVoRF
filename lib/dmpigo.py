...
from . import grid
from .dvgo import Raw2Alpha, Alphas2Weights, render_utils_cuda, reliable_based_smooth_cuda
...
class DirectMPIGO(torch.nn.Module):
    ...
    def _set_grid_resolution(self, num_voxels, mpi_depth):
        ...
        r = (num_voxels / self.mpi_depth / ((self.xyz_max - self.xyz_min)[0]*(self.xyz_max - self.xyz_min))[1]).sqrt()
        ...
        self.z_voxel_size = (self.xyz_max - self.xyz_min)[2]/self.mpi_depth
    ...
    def reliable_count_views(self, rays_o_tr, rays_d_tr, masks, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        ...
        for rays_o_, rays_d_, mask_ in zip(rays_o_tr.split(imsz), rays_d_tr.split(imsz), masks.split(imsz)):
            ...
            for rays_o, rays_d in zip(rays_o_, rays_d_):
                ...
                step = stepsize * self.z_voxel_size * rng
        ...
        print('dmpigo: reliable_count_views finish (eps time:', eps_time, 'sec)')
        return count
    ...
    def reliable_based_k0_smooth_add_grad(self,weight,dense_mode):
        wxy = weight * self.world_size[:2].max() / 128
        wz = weight * self.mpi_depth / 128
        reliable_based_smooth_cuda.reliable_based_smooth(self.reliable_area.repeat(1,self.k0.grid.shape[1],1,1,1).detach(),self.k0.grid,self.k0.grid.grad,wxy,wxy,wz,dense_mode)

    def reliable_based_density_smooth_add_grad(self,weight,dense_mode):
        wxy = weight * self.world_size[:2].max() / 128
        wz = weight * self.mpi_depth / 128
        reliable_based_smooth_cuda.reliable_based_smooth(self.reliable_area.detach(),self.density.grid,self.density.grid.grad,wxy,wxy,wz,dense_mode)
