...
reliable_based_smooth_cuda = load(
        name='reliable_based_smooth_cuda',
        sources=[
            os.path.join(parent_dir, path)
            for path in ['cuda/reliable_based_smooth_cuda.cu', 'cuda/reliable_based_smooth_cuda.cpp']],
        verbose=True)
...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.voxel_size_base = ((self.xyz_max - self.xyz_min)[0]*(self.xyz_max - self.xyz_min)[1]*(self.xyz_max - self.xyz_min)[2] / self.num_voxels_base).pow(1/3)
        ...
        self.reliable_area=torch.ones_like(self.density.get_dense_grid())

    def _set_grid_resolution(self, num_voxels):
        # Determine grid resolution
        self.num_voxels = num_voxels
        self.voxel_size = ((self.xyz_max - self.xyz_min)[0]*(self.xyz_max - self.xyz_min)[1]*(self.xyz_max - self.xyz_min)[2] / num_voxels).pow(1/3)
    ...
    def reliable_count_views(self, rays_o_tr, rays_d_tr, masks, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        print('dvgo: reliable_count_views start')
        far = 1e9  # the given far can be too small while rays stop when hitting scene bbox
        eps_time = time.time()
        N_samples = int(np.linalg.norm(np.array(self.world_size.cpu())+1) / stepsize) + 1
        rng = torch.arange(N_samples)[None].float()
        ...
        for rays_o_, rays_d_, mask_ in zip(rays_o_tr.split(imsz), rays_d_tr.split(imsz), masks.split(imsz)):
            ones = grid.DenseGrid(1, self.world_size, self.xyz_min, self.xyz_max)
            
            rays_o_ *= mask_
            rays_d_ *= mask_
            rays_o_mask = (torch.abs(rays_o_[..., -1])) != 0 & (torch.abs(rays_o_[..., 0]) != 0) & (torch.abs(rays_o_[..., 1]) != 0)
            rays_d_mask = (torch.abs(rays_o_[..., -1])) != 0 & (torch.abs(rays_o_[..., 0]) != 0) & (torch.abs(rays_o_[..., 1]) != 0)
            
            rays_o_ = rays_o_[rays_o_mask]
            rays_d_ = rays_d_[rays_d_mask]
            ...
            with torch.no_grad():
                count += (ones.grid.grad > 1)
        eps_time = time.time() - eps_time
        print('dvgo: reliable_count_views finish (eps time:', eps_time, 'sec)')
        return count
    ...
    def reliable_based_k0_smooth_add_grad(self,weight,dense_mode):
        w = weight * self.world_size.max() / 128
        reliable_based_smooth_cuda.reliable_based_smooth(self.reliable_area.repeat(1,self.k0.grid.shape[1],1,1,1).detach(),self.k0.grid,self.k0.grid.grad,w,w,w,dense_mode)

    def reliable_based_density_smooth_add_grad(self,weight,dense_mode):
        w = weight * self.world_size.max() / 128
        reliable_based_smooth_cuda.reliable_based_smooth(self.reliable_area.detach(),self.density.grid,self.density.grid.grad,w,w,w,dense_mode)
        

    def set_reliable_area(self,cnt):
        normalcnt = cnt.float()/cnt.max()
        exp_cnt = torch.exp(-1*normalcnt)
        self.reliable_area = 1+exp_cnt
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if render_kwargs.get('render_depth', True):
            depth = segment_coo(
                        src=(weights * torch.norm((ray_pts-rays_o[ray_id]), dim=1)),
                        index=ray_id,
                        out=torch.zeros([N]),
                        reduce='sum')
            # alphainv_last_detach = torch.where(alphainv_last >= 0.9, torch.tensor(1), torch.tensor(0))
            depth += (alphainv_last * render_kwargs['far'])
            ret_dict.update({'depth': depth})

        return ret_dict

    # def volume_entropy(self,type='density'):
    #     if type == 'density':
    #         v = self.activate_density(self.density.grid, 1)   # [1,1,x,y,z]
    #         # x   y   z
    #         if self.mask_cache is not None:
    #             left_top_behind =    v[:,:,:-1,:-1,:-1]
    #             right_top_behind=    v[:,:,1:,:-1,:-1]
    #             left_bottom_behind = v[:,:,:-1,1:,:-1]
    #             right_bottom_behind= v[:,:,1:,1:,:-1]
    #             left_top_front  =    v[:,:,:-1,:-1,1:]
    #             right_top_front =    v[:,:,1:,:-1,1:]
    #             left_bottom_front =  v[:,:,:-1,1:,1:]
    #             right_bottom_front = v[:,:,1:,1:,1:]
    #             gather_v = torch.cat([left_top_behind,right_top_behind,left_bottom_behind,right_bottom_behind,left_top_front,right_top_front,left_bottom_front,right_bottom_front],dim=1)
    #             normalize_v = gather_v / torch.sum(gather_v+1e-10,dim=1).unsqueeze(1)
    #             entropy_v = -normalize_v*torch.log2(normalize_v+1e-10)
    #             mask=self.mask_cache.mask.repeat(1,entropy_v.shape[1],1,1,1)
    #             entropy_v = entropy_v[mask[:,:,:-1,:-1,:-1] &
    #                                     mask[:,:,1:,:-1,:-1] &
    #                                     mask[:,:,:-1,1:,:-1] &
    #                                     mask[:,:,:-1,:-1,1:] &
    #                                     mask[:,:,1:,1:,:-1] &
    ...
    #                                     mask[:,:,:-1,1:,1:] &
    #                                     mask[:,:,1:,1:,1:]]
    #             return torch.mean(entropy_v)
    #         else:
    #             return None
    
    def xyz_entropy(self,type='density'):
        if type == 'density':
            v = self.activate_density(self.density.grid, 1)
            if self.mask_cache is not None:
                v = self.mask_cache.mask*v
            total_entropy_loss=0
            for dim in range(3):               # x,y,z
                reshape_v=v.permute(0,1,(dim+1)%3+2,(dim+2)%3+2,dim+2)
                normalize_v = reshape_v/torch.sum(reshape_v+1e-10,dim=-1).unsqueeze(-1)
                entropy_v = -normalize_v*torch.log(normalize_v+1e-10)
                entropy_v =torch.mean(entropy_v)
                total_entropy_loss+=entropy_v
